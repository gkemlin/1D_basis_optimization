...
g = GroupPlot(1, 1, groupStyle="horizontal sep = 2cm, vertical sep = 1.75cm")
...
push!(g, Axis(p, title=L"\mbox{Criterion 1, }H^1\mbox{ norm: Error on energy}",
              xlabel=L"$a$",
...
p = [Plots.Linear(a_sample_ext, err_HB, legendentry=latexstring("{\\rm hermite}"),
     ...
     Plots.Linear(a_sample_ext, err_OB[3], legendentry=latexstring("a \\in \\{2.5\\}"),
                  mark="none", style="thick, dash dot dot, myviolet")]
push!(g, Axis(p, title=L"\mbox{Criterion 1, }H^1\mbox{ norm: Error on density}",
              xlabel=L"$x$",
              ymode="log", ylabel=L"\Vert\rho_a - \widetilde\rho_a\Vert_{L^1}",
              legendStyle="at={(0.95,0.05)}, anchor=south east"))
save("crit1_H1_compare_samples.pdf", g)
